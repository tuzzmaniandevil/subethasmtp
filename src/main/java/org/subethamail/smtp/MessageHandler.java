...
public interface MessageHandler {
     ...
     * rfc822-compliant email address, already validated by the server. The
     * validation is performed by the JavaMail InternetAddress.parse function,
     * according to the strict rules, which means that "many (but not all) of
     * the RFC822 syntax rules are enforced".
    ...
    /**
     * Called once for every RCPT TO during a SMTP exchange. This will occur
     * after a from() call.
     *
    ...
    /**
     * Called when the DATA part of the SMTP exchange begins. This will occur
     * after all recipient() calls are complete.
     *
     ...
     * chars. The data stream will be valid only for the duration of the call.
     *
    ...
    /**
     * Called after all other methods are completed. Note that this method will
     * be called even if the mail transaction is aborted at some point after the
     * initial from() call.
     */
    public void done();
}
